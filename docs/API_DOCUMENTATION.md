...
La API implementa siete capacidades principales de Inteligencia Artificial.
...
`/users/*` y `/ai/*` requieren el encabezado
`Authorization: Bearer <FIREBASE_ID_TOKEN>`. El backend verifica el token con
Firebase Admin y rechaza cualquier UID que no pertenezca a la sesión activa.
`GET /users/` devuelve únicamente al usuario autenticado; ya no expone el
listado global de cuentas.

Las capacidades de IA usan exclusivamente el contexto financiero de ese UID,
responden en español, se limitan a finanzas personales y producen resultados
breves según el tipo de operación.
